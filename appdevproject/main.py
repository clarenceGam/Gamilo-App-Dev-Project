...
class LoginWindow(QWidget):
    def __init__(self):
        ...
        self.setGeometry(700, 300, 360, 320)
...
class CarManager(QWidget):
    ...
    def add_car(self):
        ...
        if all(data[:5]):
            try:
                self.cursor.execute(
                    "INSERT INTO cars (make, model, year, plate, status, image_path) VALUES (?, ?, ?, ?, ?, ?)",
                    data
                )
                self.conn.commit()
    ...
    def is_plate_exists(self, plate, exclude_id=None):
        if exclude_id:
            self.cursor.execute("SELECT id FROM cars WHERE plate = ? AND id != ?", (plate, exclude_id))
        else:
            self.cursor.execute("SELECT id FROM cars WHERE plate = ?", (plate,))
...
class CarListManager(QWidget):
    def __init__(self, login_window, parent_add_window):
        ...
        self.setGeometry(500, 120, 900, 700)
    ...
    def update_car(self):
        if self.selected_car_id is not None:
            ...
            if all(data[:6]):
                self.cursor.execute(
                    "UPDATE cars SET make=?, model=?, year=?, plate=?, status=?, image_path=? WHERE id=?",
                    data
                )
                self.conn.commit()
                self.load_data()
